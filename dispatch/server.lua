...
function Dispatch.call(coords, data)
    if Config.Dispatch.Type == 'normal' then
        TriggerClientEvent('paradox_bridge:dispatch:call', -1, coords, data)
    ...
    elseif Config.Dispatch.Type == 'rcore_dispatch' then
        ...
        error(('%s is not implemented, you can implement it in paradox_bridge/dispatch/server.lua'):format(Config.Dispatch.Type))
    end
end